...
Coach Anti-Arnaque (« l'Application ») est une application mobile gratuite destinée à aider les utilisateurs à identifier des messages, e-mails et sites web potentiellement frauduleux. La protection de votre vie privée est importante pour nous. Cette politique de confidentialité explique quelles données sont collectées, comment elles sont utilisées et quels sont vos droits.

## Données collectées

### Données traitées localement sur votre appareil

L'Application traite les données suivantes **uniquement sur votre appareil** (stockage local) :

- **Messages SMS** : lorsque vous choisissez d'analyser un SMS, le contenu du message et le numéro de l'expéditeur sont lus et analysés localement. Ces données sont stockées dans une base de données locale sur votre appareil uniquement (historique des analyses).
- **Contenu d'e-mails** : lorsque vous collez ou partagez le contenu d'un e-mail pour analyse, celui-ci est traité localement sur votre appareil.
- **URLs de sites web** : lorsque vous soumettez un lien pour vérification, celui-ci est analysé localement.
- **Contacts** : l'Application peut accéder à votre liste de contacts uniquement pour déterminer si l'expéditeur d'un message est un contact connu. Aucune donnée de contact n'est stockée, transmise ou partagée.

### Données transmises à des services tiers

Pour améliorer la précision des analyses, certaines données peuvent être transmises à des services de sécurité tiers :

- **Google Safe Browsing API** : les URLs contenues dans les messages ou soumises pour vérification peuvent être envoyées à Google Safe Browsing pour vérifier si elles sont répertoriées comme malveillantes. [Politique de confidentialité de Google](https://policies.google.com/privacy)
- **VirusTotal API** : les URLs peuvent être envoyées à VirusTotal pour obtenir un score de réputation. [Politique de confidentialité de VirusTotal](https://docs.virustotal.com/docs/privacy-policy)
- **WhoisXML API** : les noms de domaine peuvent être envoyés à WhoisXML pour vérifier l'âge d'enregistrement du domaine. [Politique de confidentialité de WhoisXML](https://whoisxmlapi.com/privacy-policy)

**Important** : seules les URLs et noms de domaine sont transmis à ces services. Le contenu des messages, vos informations personnelles et vos contacts ne sont jamais transmis.
